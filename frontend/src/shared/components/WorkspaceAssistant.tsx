...
function relatedQuestions(message: string) {
  ...
  if (normalized.includes('booking') || normalized.includes('appointment') || normalized.includes('schedule')) {
    return ['How do I find available slots?', 'What can Schedule Copilot do?', 'Show me today’s bookings'];
  }
...
export default function WorkspaceAssistant() {
  ...
  return (
    <>
      {open && (
        <section className="workspace-assistant" role="dialog" aria-label="Workspace assistant">
          <header className="workspace-assistant-header">
            <div>
              ...
              <span className="workspace-assistant-kicker">LIVE DATA</span>
          ...
          <div className="workspace-assistant-messages" aria-live="polite">
            {messages.length === 0 && (
              <div className="workspace-assistant-welcome">
                ...
                <p>Instant answers read straight from your workspace: inventory, purchase orders,
                  branches, bookings and schedules. For AI scheduling proposals, use Schedule Copilot.</p>
      ...
      <button type="button" className={`workspace-assistant-launcher${open ? ' is-open' : ''}`} onClick={() => setOpen((value) => !value)} aria-label={open ? 'Close quick answers' : 'Open quick answers'}>
        <span aria-hidden="true">{open ? '×' : '✦'}</span>
        {!open && <small>Quick answers</small>}
      </button>
    </>
  );
}
